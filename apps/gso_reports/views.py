...
from .models import WorkAccomplishmentReport, SuccessIndicator, IPMTDraft
...
def preview_ipmt(request):
    ...
    from .utils import collect_ipmt_reports
    reports = collect_ipmt_reports(year, month_num, unit_filter, personnel_names)

    return render(
        request,
        "gso_office/ipmt/ipmt_preview.html",
        {
            "reports": reports,
...
def save_ipmt_draft(request):
    ...
    from apps.gso_accounts.models import User
    from .models import IPMTDraft, SuccessIndicator, Unit

    unit = Unit.objects.filter(name__iexact=unit_name).first()
    for person_name in personnel_names:
        user = User.objects.filter(first_name__iexact=person_name.split()[0]).first()
        if not user:
            continue
        ...
        for row in rows:
            ...
            draft, created = IPMTDraft.objects.update_or_create(
                personnel=user,
                unit=unit,
                month=month,
                indicator=indicator,
